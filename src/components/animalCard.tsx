...
export const AnimalCard: React.FC<AnimalCardProps> = ({ animal }) => {
  ...
  // Variáveis mantidas, mas renomeadas para evitar erro de build
  const _availabilityBaseClasses = "mt-4 rounded-full py-2 px-2 text-center font-bold text-sm text-white shadow-md";
  const _availabilityDynamicClasses = isDisponivel 
    ? "bg-green-600 "
    : "bg-red-400 ";
    ...
    <div className="m-2 w-full justify-center items-center align-center p-38">
      <div className={`flex ${isEven ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className="w-1/2 p-4 flex flex-col justify-center gap-10">
          <h3 className={`${isEven ? 'text-end' : 'text-start'} font-bold font-[Lexend Deca] text-[70px]`}>
          ...
          {animal.cor && (
            <div className={`flex ${isEven ? 'justify-end' : 'justify-start'} items-center gap-2`}>
